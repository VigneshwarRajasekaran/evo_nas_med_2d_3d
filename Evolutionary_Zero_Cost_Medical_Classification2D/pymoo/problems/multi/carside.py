...
class Carside(Problem):
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        g2 = 0.261 - 0.0159 * x[:, 0] * x[:, 1] - 0.188 * x[:, 0] * 0.345 - 0.019 * x[:, 1] * x[:, 6] + 0.0144 * x[:,
                                                                                                                 2] * x[
                                                                                                                      :,
                                                                                                                      4] + 0.08045 * x[
                                                                                                                                     :,
                                                                                                                                     5] * 0.192
        g3 = 0.214 + 0.00817 * x[:, 4] - 0.131 * x[:, 0] * 0.345 - 0.0704 * x[:, 0] * 0.192 + 0.03099 * x[:, 1] * x[:,
                                                                                                                  5] - 0.018 * x[
                                                                                                                               :,
                                                                                                                               1] * x[
                                                                                                                                    :,
        ...
        g6 = 33.86 + 2.95 * x[:, 2] - 5.057 * x[:, 0] * x[:, 1] - 11 * x[:, 1] * 0.345 - 9.98 * x[:,
                                                                                                6] * 0.345 + 22 * 0.345 * 0.192
        ...
        f1 = 1.98 + 4.9 * x[:, 0] + 6.67 * x[:, 1] + 6.98 * x[:, 2] + 4.01 * x[:, 3] + 1.78 * x[:, 4] + 0.00001 * x[:,
                                                                                                                  5] + 2.73 * x[
                                                                                                                              :,
                                                                                                                              6]
        f2 = g8
