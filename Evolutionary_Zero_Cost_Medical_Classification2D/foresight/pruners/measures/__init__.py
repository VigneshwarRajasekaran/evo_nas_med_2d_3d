...
def measure(name, bn=True, copy_net=True, force_clean=True, **impl_args):
    def make_impl(func):
        def measure_impl(net_orig,medmnist_dataset, device, *args, **kwargs):
            if copy_net:
                net = net_orig.get_prunable_copy(bn=bn).to(device)
            else:
                net = net_orig
            ret = func(net,medmnist_dataset, *args, **kwargs, **impl_args)
...
def calc_measure(name,medmnist_dataset, net, device, *args, **kwargs):
    return _measure_impls[name](net,medmnist_dataset, device, *args, **kwargs)
