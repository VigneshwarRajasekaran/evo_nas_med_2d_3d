...
class Cell(nn.Module):
  ...
  def __init__(self, genotype, C_prev_prev, C_prev, C, reduction, reduction_prev, height, width, SE=False):
...
class NetworkCIFAR(nn.Module):
  ...
  def __init__(self, C, num_classes, layers, auxiliary, genotype, drop_path_prob, mode, SE=False, height_curr=None):
    ...
    for i in range(layers):
      ...
      if i in [layers // 3, 2 * layers // 3]:
        ...
        reduction = False
      cell = Cell(genotype, C_prev_prev, C_prev, C_curr, reduction, reduction_prev, height_curr, height_curr, SE=SE)
      reduction_prev = reduction
      self.cells += [cell]
      C_prev_prev, C_prev = C_prev, cell.multiplier * C_curr
      if i == 2 * layers // 3:
