...
def find_measures_arrays(net_orig,medmnist_dataset, trainloader, dataload_info, device, measure_names=None, loss_fn=F.cross_entropy):
        ...
        try:
            ...
            for measure_name in measure_names:
                if measure_name not in measure_values:
                    val = measures.calc_measure(measure_name,medmnist_dataset, net_orig, device, inputs, targets, loss_fn=loss_fn, split_data=ds)
...
def find_measures(net_orig,                  # neural network
                  medmnist_dataset,     #a medmnist dataset
    ...
    if measures_arr is None:
        measures_arr = find_measures_arrays(net_orig,medmnist_dataset, dataloader, dataload_info, device, loss_fn=loss_fn, measure_names=measure_names)
