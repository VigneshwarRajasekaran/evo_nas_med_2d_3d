...
class NAS(Problem):
    def __init__(self, n_var=6, n_obj=5, dataset='cifar10', xl=None, xu=None,pop=None,population_size=None, number_of_generations=None, crossover_prob=None, mutation_prob=None, blocks_size=None,
                         ...
                         resume_train=None, cutout=None, multigpu_num=None,medmnist_dataset=None,is_medmnist=None,check_power_consumption=False, save_dir=None, seed=0,
                 objectives_list=None, args=None,evaluation_type=None):
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        decoded_individuals = [NetworkCIFAR(self.n_channels, n_classes, self.layers, True,
                                          decode_cell(decode_operations(individuals[i][:-1], self.pop.indexes)),
                                          True,False,
                                          self.dropout_rate, 'FP32', False) for i in
        ...
        for i in range(individuals.shape[0]):
            ...
            if self.evaluation_type == 'training':
                ...
                evaluation = 'val'
                loss = self.evaluator.train(best_combination, decoded_individuals[i], 25, hash_indv=None, grad_clip=5,
                                        evaluation=evaluation, data_flag=data_flag, output_root=output_root,
                                        num_epochs=25, gpu_ids=gpu_ids, batch_size=batch_size, is_final=False,
            ...
            if self.evaluation_type == 'training':
                self.archive_obj.append([objs[i][1],objs[i][0]])
            else:
                self.archive_obj.append([loss['params'], loss['snip']])
        ...
        out["F"] = objs
    def train_final_individual(self,individual,medmnist_dataset,is_medmnist):
        data_flag = self.medmnist_dataset
        output_root = './output'
        info = INFO[self.medmnist_dataset]
        num_epochs = 300
        ...
        decoded_individual = NetworkCIFAR(self.n_channels, n_classes, int(individual[-1]), True,
        ...
        # best_combination = self.evaluator.auto_search_daapolicy(decoded_individual, 100, hash_indv=None, grad_clip=5,
        ...
        best_combination = None
        if self.multigpu_num>1:
            decoded_individual = nn.DataParallel(decoded_individual)
...
class MOGADE(Optimizer):
    ...
    def evolve(self):
        ...
        n_partitions = 5  # You can adjust this value based on your problem characteristics
        algorithm = MOEAD(get_reference_directions("uniform", problem.n_obj, n_partitions=n_partitions),
        ...
        results = minimize(
            ...
            algorithm=algorithm,
            seed=seed,
            pop_size=pop_size,  # Assign the population size here
            save_history=True,
        ...
        plt.title('Pareto Front and Other Instances')
        plt.legend()
        plt.savefig('figure_pareto_' + self.dataset + '.png')
        plt.savefig('figure_pareto_' + self.dataset + '.pdf')
        plt.grid(True)
        # Print metrics
